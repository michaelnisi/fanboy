...
function requestSearch (term, { db, cache }, cb) {
  ...
  request('/search', term, (err, res) => {
    ...
    // If we have not received any chunks, after the pipeline ended,
    // something went wrong and we must return without touching the
    // database. Without this extra check, we cannot interpret an
    // empty results Array properly. Our JSON parser does emit errors
    // if it cannot find what it’s looking for.
    let chunksCount = 0
...
/**
 * Search iTunes for term. The two code paths are: issueing an HTTP request or
 * reading values from the cache.
 */
